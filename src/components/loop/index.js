...
import './loop.css'; // <-- corrige o import (estava .mocss)
...
export const LogoLoop = memo(function LogoLoop({
  logos = [],          // <-- valor padrão evita erro do .map
  speed = 120,
  direction = 'left',
  width = '100%',
  logoHeight = 28,
  ...
  const logoLists = useMemo(() => {
    const safe = Array.isArray(logos) ? logos : [];    // <-- proteção extra
    return Array.from({ length: copyCount }, (_, copyIndex) => (
      ...
      >
        {safe.map((item, itemIndex) => renderLogoItem(item, `${copyIndex}-${itemIndex}`))}
      </ul>
    ));
